...
def ledger_value(report: dict, key: str, default: str = "n/a") -> str:
    ledger = report.get("network_assist_ledger") or {}
    return str(ledger.get(key, default))
...
def build_html(reports: list[dict]) -> str:
    ...
    for report in reports[:50]:
        rows.append(
            ...
            f"<td>{html.escape(ledger_value(report, 'currency_generated', '0.00'))} {html.escape(ledger_value(report, 'currency', 'USD'))}</td>"
            f"<td>{html.escape(ledger_value(report, 'proof_level', 'telemetry_only'))}</td>"
            "</tr>"
        )
    ...
    table_rows = "\n".join(rows) if rows else "<tr><td colspan='9'>No drill reports available.</td></tr>"
...
<head>
  ...
  <style>
    ...
    .value {{ font-size:1.25rem; margin-top:6px; font-weight:700; }}
    table {{ width:100%; border-collapse:collapse; overflow:hidden; border-radius:16px; background:rgba(16,28,47,.86); }}
    th, td {{ border-bottom:1px solid var(--line); padding:12px 10px; text-align:left; font-size:.9rem; }}
...
<body>
  <main>
    <section class="hero">
      <div class="label">SkyGrid powered by Aura-Core</div>
      ...
      <div class="grid">
        ...
        <div class="card"><div class="label">Currency generated</div><div class="value">{html.escape(ledger_value(latest, 'currency_generated', '0.00'))} {html.escape(ledger_value(latest, 'currency', 'USD'))}</div></div>
        <div class="card"><div class="label">Proof level</div><div class="value">{html.escape(ledger_value(latest, 'proof_level', 'telemetry_only'))}</div></div>
      </div>
      <p>Advisory resilience telemetry only. Authorized endpoints, no secrets, no wallet signing, no third-party network manipulation. Currency generated remains 0.00 until verified by agreement, invoice, or settlement record.</p>
    </section>

    <h2>Recent drill telemetry</h2>
    <table>
      <thead>
        <tr><th>Started</th><th>Drill ID</th><th>Primary</th><th>Fallback</th><th>Route</th><th>Primary avg</th><th>Fallback avg</th><th>Currency</th><th>Proof</th></tr>
      </thead>
