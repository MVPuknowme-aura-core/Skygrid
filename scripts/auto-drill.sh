...
FALLBACK_URL="${FALLBACK_URL:-https://github.com/MVPuknowme-aura-core/Skygrid}"
...
probe_url() {
  local name="$1"
  local url="$2"
  local started ended http_code total_time result status
...
cat > "$REPORT_JSON" <<JSON
{
  ...
  "ended_at": "$ENDED_AT",
  "repo_boundary": "MVPuknowme-aura-core/Skygrid",
  ...
  "network_assist_ledger": {
    "service_assisted": "SkyGrid public website",
    "assist_type": "availability_probe_and_fallback_recommendation",
    "billable": false,
    "currency_generated": "0.00",
    "currency": "USD",
    "proof_level": "telemetry_only"
  },
...
cat > "$REPORT_MD" <<MD
# SkyGrid Auto Drill Report

- Drill ID: $DRILL_ID
- Started: $STARTED_AT
- Ended: $ENDED_AT
- Repo boundary: MVPuknowme-aura-core/Skygrid
- Mode: advisory_simulation
...
## Network Assist Ledger

- Service assisted: SkyGrid public website
- Assist type: availability probe and fallback recommendation
- Billable: false
- Currency generated: 0.00 USD
- Proof level: telemetry_only
